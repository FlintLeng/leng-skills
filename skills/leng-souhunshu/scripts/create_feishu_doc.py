    ...
    def get_tenant_access_token(self) -> str:
        ...
        try:
            ...
            if result.get("code") == 0:
                ...
                self.token_expires = time.time() + 7000  # token 有效期约 2 小时
                return self.token
            else:
...
def create_feishu_document(title: str, content: str, user_open_id: str = None) -> Dict[str, Any]:
    """
    创建飞书文档并写入内容
    ...
    Args:
        ...
        user_open_id: 用户 open_id（用于共享权限）
...
def main():
    parser = argparse.ArgumentParser(description="创建飞书文档并写入内容")
